...
extension JavaImplementationMacro: PeerMacro {
    static func expansion(
        ...
        let diagnoseMessage = "node attribute name \(node.attributeName)"

        context.diagnose(Diagnostic(node: declaration.root, message:  myDiagnosticMessage(message: diagnoseMessage, diagnosticID: SwiftDiagnostics.MessageID(domain: "", id: ""), severity: .warning))
        )
        ...
        for memberItem in extensionDecl.memberBlock.members {
            ...
            var signatureMethodReturnType = "JavaType.void"

            let cReturnType =
            returnType.map {
                signatureMethodReturnType = "\($0).javaType"
                return "-> \($0).JNIType"
            } ?? ""
          ...
          """
            ...
            let nativeMethodName=swiftName+"NativeFunctionCall"
            exposedMembers.append(
        """

                func \(nativeMethodName)(\(cParameters.map{ $0.description.replacingOccurrences(of: "!", with: "?") }.joined(separator: ", ")))\( cReturnType) {
                \(body)
                }
        ...
        """
            ...
            var returnDataType: String = cReturnType.trimmingCharacters(in: .whitespacesAndNewlines)
            if(returnDataType==""){
                returnDataType="-> Void"
            }
            ...
            parametersClause.parameters.map { param in
                parameterTypes.append("\(param.type).javaType")
            }
            let parameterTypeString = parameterTypes.joined(separator: ",")

            var javaSignatureString = """
        MethodSignature(
            resultType: \(signatureMethodReturnType),
            parameterTypes: [\(parameterTypeString)]
        ).mangledName
"""
...
func \(swiftName)JVMInitNativeFunctionCall(env: JNIEnvironment){
do {
let className:jclass = try \(swiftTypeName).getJNIClass(in: env)
let jniNativeMethod = JNINativeMethod(
            name: strdup("\(swiftName)"),
            signature: strdup(\(javaSignatureString)),
            fnPtr: unsafeBitCast(\(nativeMethodName) as @convention(c) (\(cParameters.map{ "_ " + $0.description.replacingOccurrences(of: "!", with: "?") }.joined(separator: ", "))) \(returnDataType), to: UnsafeMutableRawPointer.self)
        )
env.pointee?.pointee.RegisterNatives(env,className,[jniNativeMethod],1)
}catch {
print("error occured while initing jvm \\(error)")
}
}
"""
            )
        ...
        let exposedMember:String=exposedMembers.joined( )
        return ["\(raw: exposedMember)"]
    }
}
